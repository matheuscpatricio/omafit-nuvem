...
async function loadConfig(appBaseUrl: string, storeId: string) {
	...
	try {
		...
		const config = data.config || {
			link_text: "Ver meu tamanho ideal (cal├ºados)",
			widget_enabled: true,
...
function ensureModal(widgetUrl: string) {
	...
	if (!modal) {
		...
		modal.hidden = true;
		modal.innerHTML = `
      <div class="omafit-modal-card" role="dialog" aria-modal="true" aria-label="Omafit Cal├ºados">
        <button type="button" class="omafit-modal-close" aria-label="Fechar">├ù</button>
        <iframe title="Omafit Cal├ºados" src=""></iframe>
      </div>
    `;
