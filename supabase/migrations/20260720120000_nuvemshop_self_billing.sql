...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'nuvemshop_stores' AND column_name = 'stripe_customer_id'
  ) THEN
    ALTER TABLE nuvemshop_stores ADD COLUMN stripe_customer_id TEXT;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'nuvemshop_stores' AND column_name = 'stripe_subscription_id'
  ) THEN
    ALTER TABLE nuvemshop_stores ADD COLUMN stripe_subscription_id TEXT;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'nuvemshop_stores' AND column_name = 'stripe_payment_status'
  ) THEN
    ALTER TABLE nuvemshop_stores ADD COLUMN stripe_payment_status TEXT;
  END IF;
END $$;
